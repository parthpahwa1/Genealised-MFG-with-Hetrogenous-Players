...
class EpisodesBufferEntry:
    ...
    def __init__(self):
        ...
        self.prob2 = []
        self.prob3 = []
        self.dominant1_state = []
        self.dominant1_action = []
        self.dominant2_state = []
        self.dominant2_action = []


        self.terminal = False


    def append(self, view, feature, action, reward, alive, prob0=None,  prob1=None,  prob2=None,  prob3=None, dominant1_state=None, dominant1_action=None, dominant2_state=None, dominant2_action=None):
        ...
        if dominant1_state is not None:
            self.dominant1_state.append(dominant1_state)
            self.dominant1_action.append(dominant1_action)
            self.dominant2_state.append(dominant2_state)
            self.dominant2_action.append(dominant2_action)
        if not alive:
            self.terminal = True
...
class EpisodesBuffer(Buffer):
    ...
    """
    def __init__(self, use_mean=False, use_dominant=False):
        super().__init__()
        self.buffer = {}
    ...
    def push(self, **kwargs):
        ...
        if self.use_mean:
            ...
            if self.use_dominant:
                dominant1_state = kwargs['dominant1_state']
                dominant1_action = kwargs['dominant1_action']
                dominant2_state = kwargs['dominant2_state']
                dominant2_action = kwargs['dominant2_action']
            ...
            if self.use_mean:
                if self.use_dominant:
                    entry.append(
                        ...
                        prob2=prob2[i],
                        prob3=prob3[i],
                        dominant1_state=dominant1_state[i],
                        dominant1_action=dominant1_action[i],
                        dominant2_state=dominant2_state[i],
                        dominant2_action=dominant2_action[i]
                    )    
                else:
...
class AgentMemory(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, use_mean=False, use_dominant=False):
        ...
        if self.use_dominant:
            self.dominant1_state0 = MetaBuffer(obs_shape, max_len)
            self.dominant1_state1 = MetaBuffer(feat_shape, max_len)
            self.dominant1_action = MetaBuffer((), max_len)
            self.dominant2_state0 = MetaBuffer(obs_shape, max_len)
            self.dominant2_state1 = MetaBuffer(feat_shape, max_len)
            self.dominant2_action = MetaBuffer((), max_len)


    def append(self, obs0, feat0, act, reward, alive, prob0=None,  prob1=None,  prob2=None,  prob3=None, dominant1_state=None, dominant1_action=None, dominant2_state=None, dominant2_action=None):
        ...
        if self.use_dominant:
            # print('dominant1_state length:', len(dominant1_state))
            # print('dominant1_state0 shape:', dominant1_state[0].shape)
            # print('dominant1_state1 shape:', dominant1_state[1].shape)
            self.dominant1_state0.append(np.array([dominant1_state[0]]))
            self.dominant1_state1.append(np.array([dominant1_state[1]]))
            self.dominant1_action.append(np.array([dominant1_action]))

            self.dominant2_state0.append(np.array([dominant2_state[0]]))
            self.dominant2_state1.append(np.array([dominant2_state[1]]))
            self.dominant2_action.append(np.array([dominant2_action]))
    ...
    def pull(self):
        res = {
            ...
            'dominant1_state0': None if not self.use_dominant else self.dominant1_state0.pull(),
            'dominant1_state1': None if not self.use_dominant else self.dominant1_state1.pull(),
            'dominant1_action': None if not self.use_dominant else self.dominant1_action.pull(),
            'dominant2_state0': None if not self.use_dominant else self.dominant2_state0.pull(),
            'dominant2_state1': None if not self.use_dominant else self.dominant2_state1.pull(),
            'dominant2_action': None if not self.use_dominant else self.dominant2_action.pull()
        }
...
class MemoryGroup(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, batch_size, sub_len, use_mean=False, use_dominant=True):
        self.agent = dict()
        self.max_len = max_len
    ...
    def _flush(self, **kwargs):
        ...
        if self.use_dominant:
            self.dominant1_state0.append(kwargs['dominant1_state0'])
            self.dominant1_state1.append(kwargs['dominant1_state1'])
            self.dominant1_action.append(kwargs['dominant1_action'])


            self.dominant2_state0.append(kwargs['dominant2_state0'])
            self.dominant2_state1.append(kwargs['dominant2_state1'])
            self.dominant2_action.append(kwargs['dominant2_action'])
    ...
    def push(self, **kwargs):
        for i, _id in enumerate(kwargs['ids']):
            if self.agent.get(_id) is None:
                self.agent[_id] = AgentMemory(self.obs_shape, self.feat_shape, self.act_n, self.sub_len, use_mean=self.use_mean, use_dominant=self.use_dominant)
            if self.use_mean:
                if self.use_dominant:
                    self.agent[_id].append(
                        ...
                        prob0=kwargs['prob0'][i],
                        prob1=kwargs['prob1'][i],
                        prob2=kwargs['prob2'][i],
                        prob3=kwargs['prob3'][i],
                        dominant1_state = [kwargs['dominant1_state'][0][0], kwargs['dominant1_state'][1][0]],
                        dominant1_action = kwargs['dominant1_action'][0],
                        dominant2_state = [kwargs['dominant2_state'][0][0], kwargs['dominant2_state'][1][0]],
                        dominant2_action = kwargs['dominant2_action'][0]
                    )
                else:
    ...
    def sample(self):
        ...
        if self.use_mean:
            ...
            if self.use_dominant:
                dominant1_state0 = self.dominant1_state0.sample(idx)
                dominant1_state1 = self.dominant1_state1.sample(idx)
                dominant1_action = self.dominant1_action.sample(idx)

                dominant2_state0 = self.dominant2_state0.sample(idx)
                dominant2_state1 = self.dominant2_state1.sample(idx)
                dominant2_action = self.dominant2_action.sample(idx)

                dominant1_next_state0 = self.dominant1_state0.sample(idx)
                dominant1_next_state1 = self.dominant1_state1.sample(idx)
                dominant1_next_action = self.dominant1_action.sample(idx)

                dominant2_next_state0 = self.dominant2_state0.sample(idx)
                dominant2_next_state1 = self.dominant2_state1.sample(idx)
                dominant2_next_action = self.dominant2_action.sample(idx)
                ...
                return [
                    ...
                    act_next_prob2,
                    act_next_prob3, 
                    dominant1_next_state0,
                    dominant1_next_state1,
                    dominant1_next_action,
                    dominant2_next_state0,
                    dominant2_next_state1,
                    dominant2_next_action,
                    rewards,
                    dones,
                    masks
                ]
