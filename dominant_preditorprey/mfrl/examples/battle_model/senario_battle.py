...
def generate_map(env, map_size, handles):
    ...
    for x in range(width//2 + gap, width//2 + gap + side, 2):
        for y in range((height - side)//2, (height - side)//2 + side, 2):
            pos[ct % 2].append([x, y])
        ct += 1
    env.add_agents(handles[3], method="custom", pos=pos[0])

    env.add_agents(handles[0], method="custom", pos=[[width//2, (height - side)//2]])
    env.add_agents(handles[1], method="custom", pos=[[width//2, (height - side)//2 + side]])
...
def play2(env, n_round, map_size, max_steps, handles, models, print_every, mtmfq_position = 0, eps=1.0, render=False, train=False):
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            if i == 0:
                print('former_act_prob0:', former_act_prob0[i].shape, 'state[i]', len(state[i]), state[i][0].shape, state[i][1].shape)
                print('state0:', state[i][0])
                print('state1:', state[i][1])
...
def play3(env, n_round, map_size, max_steps, handles, models, print_every, mtmfq_position = 0, eps=1.0, render=False, train=False):
    ...
    n_group = len(handles)
    state = [None for _ in range(n_group)]
    dominant1_state = [None for _ in range(2)]
    dominant2_state = [None for _ in range(2)]

    acts = [0 for _ in range(n_group)]
    dominant1_action = [None for _ in range(2)]
    dominant2_action = [None for _ in range(2)]
    ...
    former_act_prob = [
        np.zeros((1, env.get_action_space(handles[0])[0])),
        np.zeros((1, env.get_action_space(handles[1])[0])),
        np.zeros((1, env.get_action_space(handles[2])[0])),
        np.zeros((1, env.get_action_space(handles[3])[0]))
    ]
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            if models[i].isDom == False:
                # print(np.repeat(state[1][1], repeats=len(state[i][0]), axis=0).shape)
                acts[i] = models[i].act(
                    state=state[i],
                    ...
                    dominant1_state0=np.repeat(state[0][0], repeats=len(state[i][0]), axis=0),
                    dominant1_state1=np.repeat(state[0][1], repeats=len(state[i][0]), axis=0),
                    dominant1_action=np.repeat(acts[0], repeats=len(state[i][0]), axis=0),
                    dominant2_state0=np.repeat(state[1][0], repeats=len(state[i][0]), axis=0),
                    dominant2_state1=np.repeat(state[1][1], repeats=len(state[i][0]), axis=0),
                    dominant2_action=np.repeat(acts[1], repeats=len(state[i][0]), axis=0),
                    eps=eps
                )
            else:
                acts[i] = models[i].act(state=state[i], prob0=former_act_prob0[i], prob1=former_act_prob1[i], prob2=former_act_prob2[i], prob3=former_act_prob3[i], eps=eps) 
        ...
        buffer = {
            'state': state[0], 'acts': acts[0], 'rewards': rewards[0],
            'alives': alives[0], 'ids': ids[0], 'dominant1_state':  state[0], 'dominant1_action':acts[0], 'dominant2_state':  state[1], 'dominant2_action':acts[1]
        }
        ...
        buffer = {
            'state': state[1], 'acts': acts[1], 'rewards': rewards[1],
            'alives': alives[1], 'ids': ids[1], 'dominant1_state':  state[0], 'dominant1_action':acts[0], 'dominant2_state':  state[1], 'dominant2_action':acts[1]
        }
        ...
        buffer = {
            'state': state[2], 'acts': acts[2], 'rewards': rewards[2],
            'alives': alives[2], 'ids': ids[2], 'dominant1_state':  state[0], 'dominant1_action':acts[0], 'dominant2_state':  state[1], 'dominant2_action':acts[1]
        }
        ...
        buffer = {
            'state': state[3], 'acts': acts[3], 'rewards': rewards[3],
            'alives': alives[3], 'ids': ids[3], 'dominant1_state':  state[0], 'dominant1_action':acts[0], 'dominant2_state':  state[1], 'dominant2_action':acts[1]
        }
