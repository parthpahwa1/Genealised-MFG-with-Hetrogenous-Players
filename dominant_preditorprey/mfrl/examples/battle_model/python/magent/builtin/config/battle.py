...
def get_config(map_size):
    ...
    passive = cfg.register_agent_type(
        "passive",
    ...
    dominant = cfg.register_agent_type(
        "dominant",
        {'width': 1, 'length': 1, 'hp': 150, 'speed': 2,
         'view_range': gw.CircleRange(6), 'attack_range': gw.CircleRange(2),
         'damage': 20, 'step_recover': 0.1, 'attack_in_group': True,

         'step_reward': -0.01,  'kill_reward': 0, 'dead_penalty': -1, 'attack_penalty': -0.1,
         })

    g0 = cfg.add_group(dominant)
    g1 = cfg.add_group(dominant)
    g2 = cfg.add_group(passive)
    g3 = cfg.add_group(passive)
    ...
    cfg.add_reward_rule(gw.Event(a, 'attack', b), receiver=a, value=4)
    cfg.add_reward_rule(gw.Event(a, 'attack', c), receiver=a, value=0.4)
    cfg.add_reward_rule(gw.Event(a, 'attack', d), receiver=a, value=0.4)

    cfg.add_reward_rule(gw.Event(b, 'attack', a), receiver=b, value=4)
    cfg.add_reward_rule(gw.Event(b, 'attack', c), receiver=b, value=0.4)
    cfg.add_reward_rule(gw.Event(b, 'attack', d), receiver=b, value=0.4)

    cfg.add_reward_rule(gw.Event(c, 'attack', d), receiver=c, value=0.4)
    cfg.add_reward_rule(gw.Event(c, 'attack', a), receiver=c, value=4)
    cfg.add_reward_rule(gw.Event(c, 'attack', b), receiver=c, value=4)

    cfg.add_reward_rule(gw.Event(d, 'attack', a), receiver=d, value=4)
    cfg.add_reward_rule(gw.Event(d, 'attack', b), receiver=d, value=4)
    cfg.add_reward_rule(gw.Event(d, 'attack', c), receiver=d, value=0.4)

    cfg.add_reward_rule(gw.Event(a, 'kill', b), receiver=a, value=1000)
    cfg.add_reward_rule(gw.Event(a, 'kill', c), receiver=a, value=100)
    cfg.add_reward_rule(gw.Event(a, 'kill', d), receiver=a, value=100)
    
    cfg.add_reward_rule(gw.Event(b, 'kill', a), receiver=b, value=1000)
    cfg.add_reward_rule(gw.Event(b, 'kill', c), receiver=b, value=100)
    cfg.add_reward_rule(gw.Event(b, 'kill', d), receiver=b, value=100)


    cfg.add_reward_rule(gw.Event(c, 'kill', d), receiver=c, value=100)
    cfg.add_reward_rule(gw.Event(c, 'kill', a), receiver=c, value=1000)
    cfg.add_reward_rule(gw.Event(c, 'kill', b), receiver=c, value=1000)

    cfg.add_reward_rule(gw.Event(d, 'kill', a), receiver=d, value=1000)
    cfg.add_reward_rule(gw.Event(d, 'kill', b), receiver=d, value=1000)
