...
class EpisodesBufferEntry:
    ...
    def append(self, view, feature, action, reward, alive, prob0=None,  prob1=None, prob2=None, prob3=None, type0_state=None, type1_state=None, type2_state=None, type3_state=None):
        ...
        if prob0 is not None:
            ...
            self.type0_state.append(type0_state)
            self.type1_state.append(type1_state)
            self.type2_state.append(type2_state)
            self.type3_state.append(type3_state)
...
class EpisodesBuffer(Buffer):
    ...
    def push(self, **kwargs):
        ...
        if self.use_mean:
            ...
            prob2 = kwargs['prob2']
            prob3 = kwargs['prob3']
            type0_state = kwargs['type0_state']
            type1_state = kwargs['type1_state']
            type2_state = kwargs['type2_state']
            type3_state = kwargs['type3_state']
        ...
        for i in range(len(ids)):
            ...
            if self.use_mean:
                entry.append(
                    ...
                    prob2=prob2[i],
                    prob3=prob3[i],
                    type0_state=type0_state[i],
                    type1_state=type1_state[i],
                    type2_state=type2_state[i],
                    type3_state=type3_state[i],
                )            
            else:
...
class AgentMemory(object):
    def __init__(self, obs_shape, feat_shape, act_n, max_len, use_mean=False, use_dominant=False):
        ...
        if self.use_mean:
            ...
            self.type0_state0 = MetaBuffer((256,), max_len)
            self.type0_state1 = MetaBuffer((32,), max_len)

            self.type1_state0 = MetaBuffer((256,), max_len)
            self.type1_state1 = MetaBuffer((32,), max_len)

            self.type2_state0 = MetaBuffer((256,), max_len)
            self.type2_state1 = MetaBuffer((32,), max_len)

            self.type3_state0 = MetaBuffer((256,), max_len)
            self.type3_state1 = MetaBuffer((32,), max_len)



    def append(self, obs0, feat0, act, reward, alive, prob0=None,  prob1=None, prob2=None, prob3=None, type0_state=None, type1_state=None, type2_state=None, type3_state=None):
        ...
        if self.use_mean:
            ...
            self.type0_state0.append(np.array([type0_state[0]]))
            self.type0_state1.append(np.array([type0_state[1]]))

            self.type1_state0.append(np.array([type1_state[0]]))
            self.type1_state1.append(np.array([type1_state[1]]))

            self.type2_state0.append(np.array([type2_state[0]]))
            self.type2_state1.append(np.array([type2_state[1]]))

            self.type3_state0.append(np.array([type3_state[0]]))
            self.type3_state1.append(np.array([type3_state[1]]))
    ...
    def pull(self):
        res = {
            ...
            'type0_state0': None if not self.use_mean else self.type0_state0.pull(),
            'type0_state1': None if not self.use_mean else self.type0_state1.pull(),
            'type1_state0': None if not self.use_mean else self.type1_state0.pull(),
            'type1_state1': None if not self.use_mean else self.type1_state1.pull(),

            'type2_state0': None if not self.use_mean else self.type2_state0.pull(),
            'type2_state1': None if not self.use_mean else self.type2_state1.pull(),

            'type3_state0': None if not self.use_mean else self.type3_state0.pull(),
            'type3_state1': None if not self.use_mean else self.type3_state1.pull(),
        }
...
class MemoryGroup(object):
    ...
    def _flush(self, **kwargs):
        ...
        if self.use_mean:
            ...
            self.type0_state0.append(kwargs['type0_state0'])
            self.type0_state1.append(kwargs['type0_state1'])

            self.type1_state0.append(kwargs['type1_state0'])
            self.type1_state1.append(kwargs['type1_state1'])

            self.type2_state0.append(kwargs['type2_state0'])
            self.type2_state1.append(kwargs['type2_state1'])


            self.type3_state0.append(kwargs['type3_state0'])
            self.type3_state1.append(kwargs['type3_state1'])
        ...
        for i, _id in enumerate(kwargs['ids']):
            ...
            if self.use_mean:
                self.agent[_id].append(
                    ...
                    type0_state = [kwargs['type0_state0'][i], kwargs['type0_state1'][i]],
                    type1_state = [kwargs['type1_state0'][i], kwargs['type1_state1'][i]],
                    type2_state = [kwargs['type2_state0'][i], kwargs['type2_state1'][i]],
                    type3_state = [kwargs['type3_state0'][i], kwargs['type3_state1'][i]],
                )
    ...
    def sample(self):
        ...
        if self.use_mean:
            ...
            act_prob3 = self.prob2.sample(idx)
            ...
            type0_obs = self.type0_state0.sample(idx)
            type1_obs = self.type1_state0.sample(idx)
            type2_obs = self.type2_state0.sample(idx)
            type3_obs = self.type3_state0.sample(idx)

            type0_feat = self.type0_state1.sample(idx)
            type1_feat = self.type1_state1.sample(idx)
            type2_feat = self.type2_state1.sample(idx)
            type3_feat = self.type3_state1.sample(idx)


            type0_next_obs = self.type0_state0.sample(next_idx)
            type1_next_obs = self.type1_state0.sample(next_idx)
            type2_next_obs = self.type2_state0.sample(next_idx)
            type3_next_obs = self.type3_state0.sample(next_idx)

            type0_next_feat = self.type0_state1.sample(next_idx)
            type1_next_feat = self.type1_state1.sample(next_idx)
            type2_next_feat = self.type2_state1.sample(next_idx)
            type3_next_feat = self.type3_state1.sample(next_idx)
