...
def spawn_ai(algo_name, sess, env, handle, human_name, max_steps, isDom=False):
    if algo_name == 'GenQ_MFG':
        model = GenQ_MFG(sess, human_name, handle, env, max_steps, memory_size=180000)
    elif algo_name == 'mfq':
        model = MFQ(sess, human_name, handle, env, max_steps, memory_size=180000)
    elif algo_name == 'mtmfq':
        model = MTMFQ(sess, human_name, handle, env, max_steps, memory_size=180000)
    elif algo_name == 'mfac':
        model = MFAC(sess, human_name, handle, env)
    elif algo_name == 'ac':
        model = AC(sess, human_name, handle, env)
    elif algo_name == 'il':
        model = IL(sess, human_name, handle, env, max_steps, memory_size=180000)
    return model
