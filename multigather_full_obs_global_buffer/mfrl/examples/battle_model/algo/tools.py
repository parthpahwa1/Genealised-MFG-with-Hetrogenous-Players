...
# env, n_round, map_size, max_steps, handles, model_list, adv_type_list, print_every

class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
                play_handle, adv_type_list, render_every=None, save_every=None, tau=None, log_name=None, log_dir=None, model_dir=None, train=False, global_replay_buffer=None):
        """Initialize runner
        ...
        self.train = train
        self.adv_type_list = adv_type_list
        self.global_replay_buffer = global_replay_buffer

        if self.train:
            self.summary = SummaryObj(log_name=log_name+'_main', log_dir=log_dir)
            self.summary_oppo = SummaryObj(log_name=log_name+'_oppo', log_dir=log_dir)

            summary_items = ['ave_agent_reward', 'total_reward', 'alive', "Sum_Reward", "Kill_Sum", "loss", "food_reamining"]
            self.summary.register(summary_items)  # summary register
            self.summary_oppo.register(summary_items)  # summary register
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        info['main'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0., 'loss':0., 'food_reamining': 0}
        info['opponent1'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0., 'loss':0., 'food_reamining':0}
        # info['opponent2'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0.}
        # info['opponent3'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0.}
        max_nums, nums, agent_r_records, total_rewards, loss, food_num = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    model_list =self.models, adv_type_list=self.adv_type_list, print_every=50, eps=variant_eps, render=(iteration + 1) % self.render_every if self.render_every > 0 else False, train=self.train)
        ...
        for i, tag in enumerate(['main', 'opponent1']):
            ...
            info[tag]['alive'] = nums[i]
            info[tag]['ave_agent_reward'] = agent_r_records[i]
            info[tag]['loss'] = loss[i]
            info[tag]['food_reamining'] = food_num

        if self.train:
            print('\n[INFO] {}'.format(info['main']))
            print('\n[INFO] {}'.format(info['opponent1']))
            # print('\n[INFO] {}'.format(info['opponent2']))
            # print('\n[INFO] {}'.format(info['opponent3']))
            self.summary.write(info['main'], iteration)
            self.summary_oppo.write(info['opponent1'], iteration)
            ...
            if self.save_every and (iteration + 1) % self.save_every == 0:
                ...
                # self.models[1].save(self.model_dir + self.adv_type_list[1] + '-oppo', iteration)
                # self.models[2].save(self.model_dir + self.adv_type_list[2] + '-oppo', iteration)
                # self.models[3].save(self.model_dir + self.adv_type_list[3] + '-oppo', iteration)
                # self.models[2].save(self.model_dir + '-2', iteration)
                # self.models[3].save(self.model_dir + '-3', iteration)


        else:
            print('\n[INFO] {0} \n {1}'''.format(info['main'], info['opponent1']))
            if info['main']['alive'] > info['opponent1']['alive']:
                win_cnt['main'] += 1
            elif info['opponent1']['alive'] > info['main']['alive']:
                win_cnt['opponent1'] += 1
            else:
                win_cnt['draw'] += 1
