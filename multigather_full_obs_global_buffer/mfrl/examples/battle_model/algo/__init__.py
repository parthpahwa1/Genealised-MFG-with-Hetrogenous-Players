...
def spawn_ai(algo_name, sess, env, handle, human_name, max_steps, isDom=False, mem_size=2**20):
    if algo_name == 'GenQ_MFG':
        ...
        model = MFQ(sess, human_name, handle, env, max_steps, memory_size=mem_size)
    elif algo_name == 'mtmfq':
        model = MTMFQ(sess, human_name, handle, env, max_steps, memory_size=mem_size)
    elif algo_name == 'mfac':
        model = MFAC(sess, human_name, handle, env)
    elif algo_name == 'ac':
        model = AC(sess, human_name, handle, env)
    elif algo_name == 'il':
        model = IL(sess, human_name, handle, env, max_steps, memory_size=mem_size)
    return model
