...
class Dominant_ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=True, learning_rate=1e-4, tau=0.005, gamma=0.95):
        ...
        print('Dom_Actions:', self.num_actions)
...
class Passive_ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=True, use_dominant=True, learning_rate=1e-4, tau=0.005, gamma=0.95):
        # assert isinstance(env, GridWorld)
        self.isDom = False
        self.env = env
        self.name = name
        ...
        with tf.variable_scope(name or "ValueNet"):
            ...
            self.obs_input = tf.placeholder(tf.float32, (None,) + self.view_space, name="Obs-Input")
            self.feat_input = tf.placeholder(tf.float32, (None,) + self.feature_space, name="Feat-Input")
            self.dominant1_obs_input = tf.placeholder(tf.float32, (None,) + self.view_space, name="Dom1-Obs-Input")
            self.dominant1_feat_input = tf.placeholder(tf.float32, (None,) + self.feature_space, name="Dom1-Feat-Input")
            self.dominant2_obs_input = tf.placeholder(tf.float32, (None,) + self.view_space, name="Dom2-Obs-Input")
            self.dominant2_feat_input = tf.placeholder(tf.float32, (None,) + self.feature_space, name="Dom2-Feat-Input")
            self.dominant1_action_input = tf.placeholder(tf.int32, (None, ), name="Dom1-act-Input")
            self.dominant2_action_input = tf.placeholder(tf.int32, (None,), name="Dom2-act-Input")
    ...
    def _construct_net(self, active_func=None, reuse=False):
        ...
        conv1_dom1 = tf.layers.conv2d(self.dominant1_obs_input, filters=32, kernel_size=3,
                                 activation=active_func, name="dom1-Conv1")
        conv2_dom1 = tf.layers.conv2d(conv1_dom1, filters=32, kernel_size=3, activation=active_func,
                                 name="dom1-Conv2")
        flatten_obs_dom1 = tf.reshape(conv2_dom1, [-1, np.prod([v.value for v in conv2_dom1.shape[1:]])])
        # flatten_obs_dom1 = tf.repeat(flatten_obs_dom1, repeats=flatten_obs.shape[0]//flatten_obs_dom1.shape[0], axis=0)

        conv1_dom2 = tf.layers.conv2d(self.dominant1_obs_input, filters=32, kernel_size=3,
                                 activation=active_func, name="dom2-Conv1")
        conv2_dom2 = tf.layers.conv2d(conv1_dom2, filters=32, kernel_size=3, activation=active_func,
                                 name="dom2-Conv2")
        flatten_obs_dom2 = tf.reshape(conv2_dom2, [-1, np.prod([v.value for v in conv2_dom2.shape[1:]])])
        # flatten_obs_dom2 = tf.repeat(flatten_obs_dom2, repeats=flatten_obs.shape[0]//flatten_obs_dom2.shape[0], axis=0)
        ...
        h_obs_dom1 = tf.layers.dense(flatten_obs_dom1, units=256, activation=active_func,
                                name="Dense-Obs-dom1")
        # h_obs_dom1 = tf.repeat(h_obs_dom1, repeats=h_obs.shape[0]//h_obs_dom1.shape[0], axis=0)
        
        
        h_obs_dom2 = tf.layers.dense(flatten_obs_dom2, units=256, activation=active_func,
                                name="Dense-Obs-dom2")
        # h_obs_dom2 = tf.repeat(h_obs_dom2, repeats=h_obs.shape[0]//h_obs_dom2.shape[0], axis=0)
        ...
        h_emb_dom1 = tf.layers.dense(self.dominant1_feat_input, units=32, activation=active_func,
                                name="Dense-Emb_dom1", reuse=reuse)
        # h_emb_dom1 = tf.repeat(h_emb_dom1, repeats=h_emb.shape[0]//h_emb_dom1.shape[0], axis=0)

        
        
        h_emb_dom2 = tf.layers.dense(self.dominant2_feat_input, units=32, activation=active_func,
                                name="Dense-Emb_dom2", reuse=reuse)
        # h_emb_dom2 = tf.repeat(h_emb_dom2, repeats=h_emb.shape[0]//h_emb_dom2.shape[0], axis=0)

        
        concat_layer = tf.concat([h_obs, h_obs_dom1, h_obs_dom2, h_emb, h_emb_dom1, h_emb_dom2], axis=1)

        if self.use_mf:
            dom1_act_emb = tf.layers.dense(tf.one_hot(self.dominant1_action_input, depth=self.num_actions, on_value=1.0, off_value=0.0), units=64, activation=active_func, name='Dom1-act-Emb')
            h_act_prob = tf.layers.dense(dom1_act_emb, units=32, activation=active_func, name="Dom1-Dense-Act")
            # h_act_prob = tf.repeat(h_act_prob, repeats=h_emb.shape[0]//h_act_prob.shape[0], axis=0)
            concat_layer = tf.concat([concat_layer, h_act_prob], axis=1)

            dom2_act_emb = tf.layers.dense(tf.one_hot(self.dominant2_action_input, depth=self.num_actions, on_value=1.0, off_value=0.0), units=64, activation=active_func, name='Dom2-act-Emb')
            h_act_prob = tf.layers.dense(dom2_act_emb, units=32, activation=active_func, name="Dom2-Dense-Act")
            # h_act_prob = tf.repeat(h_act_prob, repeats=h_emb.shape[0]//h_act_prob.shape[0], axis=0)
            concat_layer = tf.concat([concat_layer, h_act_prob], axis=1)
    ...
    def calc_target_q(self, **kwargs):
        ...
        feed_dict = {
            self.obs_input: kwargs['obs'],
            self.feat_input: kwargs['feature'],
            self.dominant1_obs_input: kwargs['dominant1_state0'],
            self.dominant1_feat_input: kwargs['dominant1_state1'],
            self.dominant1_action_input: kwargs['dominant1_action'],

            self.dominant2_obs_input: kwargs['dominant2_state0'],
            self.dominant2_feat_input: kwargs['dominant2_state1'],
            self.dominant2_action_input: kwargs['dominant2_action'],
        }
