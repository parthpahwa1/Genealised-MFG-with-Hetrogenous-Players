...
from examples.battle_model.senario_battle import play_multi 
...
if __name__ == '__main__':
    ...
    models = [
        spawn_ai(args.algo, sess, env, handles[0], args.algo + '-main', args.max_steps),
        spawn_ai('mfq', sess, env, handles[1], args.algo + '_mfq' + '-oppo1', args.max_steps),
        spawn_ai('il', sess, env, handles[1], args.algo + '_il' + '-oppo2', args.max_steps),
        spawn_ai('mtmfq', sess, env, handles[1], args.algo + '_mtmfq' + '-oppo3', args.max_steps),
        spawn_ai('GenQ_MFG', sess, env, handles[1], args.algo + '_GenQ_MFG' + '-oppo4', args.max_steps)
    ]
    adv_type_list = [args.algo, 'mfq', 'il', 'mtmfq', 'GenQ_MFG']
    sess.run(tf.global_variables_initializer())
    

    runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps, models, play_multi, adv_type_list=adv_type_list,
    ...
    for k in range(start_from, start_from + args.n_round):
        ...
        with open('multibattle_{0}.csv'.format(args.algo), 'a') as myfile:
