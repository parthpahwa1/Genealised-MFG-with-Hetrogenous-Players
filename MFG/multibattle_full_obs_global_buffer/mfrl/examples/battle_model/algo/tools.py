...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
                play_handle, adv_type_list, render_every=None, save_every=None, tau=None, log_name=None, log_dir=None, model_dir=None, train=False):
        """Initialize runner
        ...
        if self.train:
            ...
            summary_items = ['ave_agent_reward', 'total_reward', 'alive', "Sum_Reward", "Kill_Sum", "loss"]
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        # print('Num agents in handle', self.handles[0], self.env.get_num(self.handles[0]))
        # print('Num agents in handle', self.handles[1], self.env.get_num(self.handles[1]))
        # print('Num agents in handle', self.handles[2], self.env.get_num(self.handles[2]))
        # print('Num agents in handle', self.handles[3], self.env.get_num(self.handles[3]))
        # pass
        info['main'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0., 'loss':0.}
        info['opponent1'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0., 'loss':0.}
        # info['opponent2'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0.}
        # info['opponent3'] = {'ave_agent_reward': 0., 'total_reward': 0., 'alive': 0.}
        max_nums, nums, agent_r_records, total_rewards, loss = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
        ...
        if self.train:
            ...
            if self.save_every and (iteration + 1) % self.save_every == 0:
                ...
                self.summary.write(info['opponent1'], iteration)
