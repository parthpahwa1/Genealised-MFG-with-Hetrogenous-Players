...
def play(env, n_round, map_size, max_steps, handles, models, print_every, eps=1.0, mtmfq_position = 0, GenQ_MFG_position=1, render=False, train=False):
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            acts[i], _, _ = models[i].act(state=state[i], prob=former_act_prob[i], eps=eps) 
    ...
    if train:
        loss[0] = models[0].train()
        loss[1] = models[1].train()
        loss[2] = models[2].train()
        loss[3] = models[3].train()
...
def play2(env, n_round, map_size, max_steps, handles, models, print_every, mtmfq_position = 0, GenQ_MFG_position=1, eps=1.0, render=False, train=False):
    """play a round and train"""
    env.reset()
    generate_map(env, map_size, handles)
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            acts[i], _, _ = models[i].act(state=state[i], prob0=former_act_prob0[i], prob1=former_act_prob1[i], prob2=former_act_prob2[i], prob3=former_act_prob3[i], eps=eps) 
...
def play3(env, n_round, map_size, max_steps, handles, models, print_every, mtmfq_position = 0, GenQ_MFG_position=1, eps=1.0, render=False, train=False):
    """play a round and train"""
    env.reset()
    generate_map(env, map_size, handles)
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            former_obs_prob0[i] = np.tile(former_obs_prob[0], (len(state[i][0]), 1)) 
            former_obs_prob1[i] = np.tile(former_obs_prob[1], (len(state[i][0]), 1)) 
            former_obs_prob2[i] = np.tile(former_obs_prob[2], (len(state[i][0]), 1)) 
            former_obs_prob3[i] = np.tile(former_obs_prob[3], (len(state[i][0]), 1))


            former_feat_prob0[i] = np.tile(former_feat_prob[0], (len(state[i][0]), 1)) 
            former_feat_prob1[i] = np.tile(former_feat_prob[1], (len(state[i][0]), 1)) 
            former_feat_prob2[i] = np.tile(former_feat_prob[2], (len(state[i][0]), 1)) 
            former_feat_prob3[i] = np.tile(former_feat_prob[3], (len(state[i][0]), 1))


            acts[i], mean_obs[i], mean_feat[i] = models[i].act(
                state=state[i],

                prob0=former_act_prob0[i],
                prob1=former_act_prob1[i],
                prob2=former_act_prob2[i],
                prob3=former_act_prob3[i],

                type0_state0=former_obs_prob0[i],
                type0_state1=former_feat_prob0[i],
                
                type1_state0=former_obs_prob1[i],
                type1_state1=former_feat_prob1[i],

                type2_state0=former_obs_prob2[i],
                type2_state1=former_feat_prob2[i],

                type3_state0=former_obs_prob3[i],
                type3_state1=former_feat_prob3[i],
                eps=eps
            )
        ...
        buffer['type0_state0'] = former_obs_prob0[0]
        buffer['type0_state1'] = former_feat_prob0[0]

        buffer['type1_state0'] = former_obs_prob1[0]
        buffer['type1_state1'] = former_feat_prob1[0]

        buffer['type2_state0'] = former_obs_prob2[0]
        buffer['type2_state1'] = former_feat_prob2[0]

        buffer['type3_state0'] = former_obs_prob3[0]
        buffer['type3_state1'] = former_feat_prob3[0]
        ...
        buffer['type0_state0'] = former_obs_prob0[1]
        buffer['type0_state1'] = former_feat_prob0[1]

        buffer['type1_state0'] = former_obs_prob1[1]
        buffer['type1_state1'] = former_feat_prob1[1]

        buffer['type2_state0'] = former_obs_prob2[1]
        buffer['type2_state1'] = former_feat_prob2[1]

        buffer['type3_state0'] = former_obs_prob3[1]
        buffer['type3_state1'] = former_feat_prob3[1]
        ...
        buffer['type0_state0'] = former_obs_prob0[2]
        buffer['type0_state1'] = former_feat_prob0[2]

        buffer['type1_state0'] = former_obs_prob1[2]
        buffer['type1_state1'] = former_feat_prob1[2]

        buffer['type2_state0'] = former_obs_prob2[2]
        buffer['type2_state1'] = former_feat_prob2[2]

        buffer['type3_state0'] = former_obs_prob3[2]
        buffer['type3_state1'] = former_feat_prob3[2]
        ...
        buffer['type0_state0'] = former_obs_prob0[3]
        buffer['type0_state1'] = former_feat_prob0[3]

        buffer['type1_state0'] = former_obs_prob1[3]
        buffer['type1_state1'] = former_feat_prob1[3]

        buffer['type2_state0'] = former_obs_prob2[3]
        buffer['type2_state1'] = former_feat_prob2[3]

        buffer['type3_state0'] = former_obs_prob3[3]
        buffer['type3_state1'] = former_feat_prob3[3]
        ...
        for i in range(n_group):
            former_feat_prob[i] = np.mean(mean_feat[i], axis=0, keepdims=True)
            former_obs_prob[i] = np.mean(mean_obs[i], axis=0, keepdims=True)
...
def battle(env, n_round, map_size, max_steps, handles, models, print_every, mtmfq_position=0, GenQ_MFG_position=1, eps=1.0, render=False, train=False):
    """play a round and train"""
    env.reset()
    ...
    while not done and step_ct < max_steps:
        ...
        for i in range(n_group):
            ...
            elif i == mtmfq_position:
                former_act_prob0[i] = np.tile(former_act_prob[0], (len(state[i][0]), 1)) 
                former_act_prob1[i] = np.tile(former_act_prob[1], (len(state[i][0]), 1)) 
                acts[i], mean_obs[i], mean_feat[i] = models[i].act(state=state[i], prob0=former_act_prob0[i], prob1=former_act_prob1[i], prob2=former_act_prob0[i],prob3=former_act_prob1[i],eps=eps)

            else: 
                former_act_prob_new[i] = np.tile(former_act_prob_new[i], (len(state[i][0]), 1))
                acts[i], mean_obs[i], mean_feat[i] = models[i].act(state=state[i], prob=former_act_prob_new[i], eps=eps)
