...
class DQN(base.ValueNet):
    def __init__(self, sess, name, handle, env, sub_len, memory_size=2**11, batch_size=512, update_every=5):
...
class MFQ(base.ValueNet):
    def __init__(self, sess, name, handle, env, sub_len, eps=1.0, update_every=5, memory_size=2**11, batch_size=512):
...
class MTMFQ(base2.ValueNet):
    def __init__(self, sess, name, handle, env, sub_len, eps=1.0, update_every=5, memory_size=2**10, batch_size=512):
        super().__init__(sess, env, handle, name, use_mf=True, update_every=update_every)
...
class GenQ_MFG(base3.FullObserve_ValueNet):
    ...
    def train(self):
        ...
        for i in range(batch_name):
            ...
            type0_obs = data_sample[7]
            type1_obs = data_sample[8]
            type2_obs = data_sample[9]
            type3_obs = data_sample[10]


            type0_feat = data_sample[11]
            type1_feat = data_sample[12]
            type2_feat = data_sample[13]
            type3_feat = data_sample[14]

            obs_next  = data_sample[15]
            feat_next = data_sample[16]
            
            act_prob_next0 = data_sample[17]
            act_prob_next1 = data_sample[18]
            act_prob_next2 = data_sample[19]
            act_prob_next3 = data_sample[20]

            type0_next_obs = data_sample[21]
            type1_next_obs = data_sample[22]
            type2_next_obs = data_sample[23]
            type3_next_obs = data_sample[24]
            
            type0_next_feat = data_sample[25]
            type1_next_feat = data_sample[26]
            type2_next_feat = data_sample[27]
            type3_next_feat = data_sample[28]

            rewards = data_sample[29]
            dones = data_sample[30]
            masks = data_sample[31]
            ...
            target_q = self.calc_target_q(
                ...
                type2_state0=type2_next_obs,
                type2_state1=type2_next_feat,
                type3_state0=type3_next_obs,
                type3_state1=type3_next_feat,
            )
            ...
            loss, q = super().train(
                ...
                type2_state0=type2_obs,
                type2_state1=type2_feat,
                type3_state0=type3_obs,
                type3_state1=type3_feat,
                masks=masks
            )
            total_loss += loss
            self.update()
