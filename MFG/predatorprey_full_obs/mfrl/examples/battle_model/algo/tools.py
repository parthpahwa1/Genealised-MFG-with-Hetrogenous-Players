...
class Runner(object):
    def __init__(self, sess, env, handles, map_size, max_steps, models,
                play_handle, mtmfq_position = 0, GenQ_MFG_position=1, render_every=None, save_every=None, tau=None, log_name=None, log_dir=None, model_dir=None, train=False):
        """Initialize runner
        ...
        self.GenQ_MFG_position = GenQ_MFG_position
    ...
    def run(self, variant_eps, iteration, win_cnt=None):
        ...
        max_nums, nums, agent_r_records, total_rewards, loss = self.play(env=self.env, n_round=iteration, map_size=self.map_size, max_steps=self.max_steps, handles=self.handles,
                    models=self.models, mtmfq_position=self.mtmfq_position, GenQ_MFG_position=self.GenQ_MFG_position, print_every=50, eps=variant_eps, render=(iteration + 1) % self.render_every if self.render_every > 0 else False, train=self.train)
        ...
        if self.train:
            ...
            if info['main']['alive'] >= info['opponent1']['alive']:
                win_cnt['main'] += 1
            if info['opponent1']['alive'] >= info['main']['alive']:
                win_cnt['opponent1'] += 1
